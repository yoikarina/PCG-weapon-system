...
public class WeaponAlignmentTool : EditorWindow
{
    ...
    {
        GetWindow<WeaponAlignmentTool>("Weapon Workbench").minSize = new Vector2(850, 550);
    }
    ...
    private void OnEnable()
    {
        ...
        WeaponToolSettings.LoadSettings();
        LoadLibrary();
    ...
    private void DrawCalibrationUI()
    {
        ...
        bool newAssistState = EditorGUILayout.ToggleLeft("👁️ Show 3D Visual Assist", showCalibrationAssist, EditorStyles.boldLabel);
    ...
    private void RandomizeEquipAssembly()
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            List<GameObject> validAssets = new List<GameObject>();
            foreach (GameObject obj in assetLibrary[i])
            {
    ...
    private void HandleDragAndDrop(Rect dropArea)
    {
        ...
        if (evt.type == EventType.DragUpdated || evt.type == EventType.DragPerform)
        {
            ...
            if (evt.type == EventType.DragPerform)
            {
                ...
                {
                    string path = AssetDatabase.GetAssetPath(draggedObj).ToLower();
                    if (draggedObj is GameObject go && (PrefabUtility.IsPartOfPrefabAsset(go) || path.EndsWith(".fbx") || path.EndsWith(".obj")))
                    {
    ...
    {
        if (WeaponToolSettings.dummyBody == null || WeaponToolSettings.dummyMuzzle == null)
        {
            if (EditorUtility.DisplayDialog("Missing Dummy", "Please configure base dummies first!\nGo to settings now?", "Settings", "Cancel")) WeaponToolSettings.ShowWindow();
            return;
        }
        ...
        if (tabIndex == 0) // Receiver Calibration
        {
            ...
            GameObject mDummy = SpawnDummy(WeaponToolSettings.dummyMuzzle, "Socket_Muzzle");
            GameObject oDummy = SpawnDummy(WeaponToolSettings.dummyScope, "Socket_Optic");
            GameObject sDummy = SpawnDummy(WeaponToolSettings.dummyStock, "Socket_Stock");
            GameObject magDummy = SpawnDummy(WeaponToolSettings.dummyMag, "Socket_Magazine");
        ...
        {
            SpawnDummy(WeaponToolSettings.dummyBody, "Body");
    ...
    private void CompleteCalibration()
    {
        ...
        if (success)
        {
            ...
            if (savedPrefab != null)
            {
                ...
                if (index >= 0) assetLibrary[selectedTab][index] = savedPrefab;
                calibrationStatus[savedPrefab] = true;
                SaveLibrary();
    ...
    private void RefreshEquipAssembly()
    {
        ...
        for (int i = 1; i <= 4; i++)
        {
            ...
            {
                GameObject accInstance = (GameObject)PrefabUtility.InstantiatePrefab(equipLoadout[i]);
    ...
    private void SaveLibrary()
    {
        for (int i = 0; i < assetLibrary.Length; i++)
        {
            ...
            foreach (GameObject go in assetLibrary[i])
            {
                if (go != null)
                {
