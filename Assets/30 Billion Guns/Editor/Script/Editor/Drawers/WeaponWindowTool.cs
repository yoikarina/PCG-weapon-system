...
{
    // ── Folder constants ──────────────────────────────────────────────────────
    private const string GUNBODY_PATH = "Assets/30 Billion Guns/Data/Gunbody";
    private const string ATTACHMENT_PATH = "Assets/30 Billion Guns/Data/Attachment";
    private const string REGISTRY_PATH = "Assets/30 Billion Guns/Data/Registry";
    private const string TAGS_PATH = "Assets/30 Billion Guns/Data/Tags";
    ...
    private void DrawCombinedStats()
    {
        ...
        if (stats.custom != null)
            foreach (var kv in stats.custom)
    ...
    private void WB_InfoBox(string text)
    {
        GUILayout.Space(2);
        GUIStyle s = new GUIStyle(EditorStyles.miniLabel)
            { wordWrap = true, normal = { textColor = C_TEXT_DIM },
    ...
    private void WB_AssetTitle(string name, string badge)
    {
        ...
        GUIStyle ts = new GUIStyle(EditorStyles.boldLabel)
            { fontSize = 13, normal = { textColor = C_TEXT_MAIN },
              alignment = TextAnchor.MiddleLeft };
    ...
    private void DrawStatList(List<GunAssemblyTool.StatEntry> statList, UnityEngine.Object owner)
    {
        ...
        else
        {
            ...
            {
                GUILayout.BeginHorizontal();
                GUILayout.Label(statList[i].key, GUILayout.Width(110));
        ...
        // Combined dropdown: presets + saved custom keys
        ...
        if (allDropdown.Length > 0)
        {
            ...
            _presetStatIdx = EditorGUILayout.Popup(_presetStatIdx, allDropdown, GUILayout.Width(130));
            if (GUILayout.Button("+ Add", EditorStyles.miniButton, GUILayout.Width(50)))
            {
        ...
        // Add custom stat
        GUILayout.BeginHorizontal();
        _customStatKey = EditorGUILayout.TextField(_customStatKey, GUILayout.Width(130));
        ...
        // Manage saved custom keys (collapsible)
